...
public class AppointmentSpecificationTest {
  ...
  private Appointment buildAppointment(
      Doctor doctor, Patient patient, LocalDate date, AppointmentStatus status) {
  ...
  void filter_shouldReturnAllAppointments_whenNoFiltersProvided() {
    ...
    appointmentRepository.save(
        buildAppointment(doctor, patient, LocalDate.of(2026, 10, 1), AppointmentStatus.BOOKED));

    List<Appointment> results =
        appointmentRepository.findAll(AppointmentSpecification.filter(new AppointmentFilter()));
  ...
  void filter_shouldMatchDoctorId() {
    ...
    appointmentRepository.save(
        buildAppointment(
            matchingDoctor, patient, LocalDate.of(2026, 10, 1), AppointmentStatus.BOOKED));
    appointmentRepository.save(
        buildAppointment(
            otherDoctor, patient, LocalDate.of(2026, 10, 1), AppointmentStatus.BOOKED));

    AppointmentFilter filter = new AppointmentFilter();
    filter.setDoctorId(matchingDoctor.getId());

    List<Appointment> results =
        appointmentRepository.findAll(AppointmentSpecification.filter(filter));

    assertThat(results)
        .extracting(appointment -> appointment.getDoctor().getId())
  ...
  void filter_shouldMatchPatientId() {
    ...
    appointmentRepository.save(
        buildAppointment(
            doctor, matchingPatient, LocalDate.of(2026, 10, 1), AppointmentStatus.BOOKED));
    appointmentRepository.save(
        buildAppointment(
            doctor, otherPatient, LocalDate.of(2026, 10, 1), AppointmentStatus.BOOKED));
    ...
    assertThat(results)
        .extracting(appointment -> appointment.getPatient().getId())
  ...
  void filter_shouldMatchDate() {
    ...
    appointmentRepository.save(
        buildAppointment(doctor, patient, matchingDate, AppointmentStatus.BOOKED));
    appointmentRepository.save(
        buildAppointment(doctor, patient, matchingDate.plusDays(1), AppointmentStatus.BOOKED));
  ...
  void filter_shouldMatchStatus() {
    ...
    appointmentRepository.save(
        buildAppointment(doctor, patient, LocalDate.of(2026, 10, 1), AppointmentStatus.CONFIRMED));
    appointmentRepository.save(
        buildAppointment(doctor, patient, LocalDate.of(2026, 10, 2), AppointmentStatus.CANCELLED));
  ...
  void filter_shouldMatchAllProvidedFilters() {
    ...
    appointmentRepository.save(
        buildAppointment(
            matchingDoctor, matchingPatient, matchingDate, AppointmentStatus.CONFIRMED));
    appointmentRepository.save(
        buildAppointment(otherDoctor, otherPatient, matchingDate, AppointmentStatus.CONFIRMED));
