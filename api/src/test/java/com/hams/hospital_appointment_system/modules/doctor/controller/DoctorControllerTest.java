...
class DoctorControllerTest {
  ...
  void getAllDoctors_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/doctors"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched all doctors successfully."))
  ...
  void createDoctor_shouldReturnOk() throws Exception {
    ...
    when(doctorService.createDoctor(any(DoctorRequest.class))).thenReturn(response);
  ...
  void getDoctorById_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/doctors/{id}", doctorId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched doctor successfully."));
  ...
  void updateDoctor_shouldReturnOk() throws Exception {
    ...
    when(doctorService.updateDoctor(eq(doctorId), any(DoctorRequest.class))).thenReturn(response);
  ...
  void getByFilter_shouldReturnOk() throws Exception {
    ...
    when(doctorService.getDoctorsByFilter(any())).thenReturn(List.of(response));

    mockMvc
        .perform(get("/api/v1/doctors/filter").param("specialization", "Cardiology"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched doctors by filter successfully."))
        .andExpect(jsonPath("$.data").isArray())
