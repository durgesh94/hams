...
class UserControllerTest {
  ...
  void getUserById_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/users/id").param("id", String.valueOf(userId)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched user successfully"));
  ...
  void getUserByUsername_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/users/username").param("username", username))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched user successfully"));
