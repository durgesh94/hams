...
public class AppointmentControllerTest {
  ...
  void getAppointmentById_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/appointments/{id}", 1L))
  ...
  void updateAppointmentStatus_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/appointments/{id}/status", 1L).with(csrf()).param("status", "CONFIRMED"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getAppointments_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/appointments/filter")
                .param("doctorId", "1")
                .param("status", "BOOKED")
                .param("page", "0")
                .param("size", "10"))
  ...
  void deleteAppointment_shouldReturnNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/v1/appointments/{id}", 1L).with(csrf()))
