...
class PatientControllerTest {
  ...
  void getAllPatients_shouldReturnOk() throws Exception {
    ...
    when(patientService.getAllPatients()).thenReturn(List.of(response1, response2));

    mockMvc
        .perform(get("/api/v1/patients"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.message").value("Fetched all patients"))
  ...
  void getPatientById_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/patients/{id}", patientId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(200))
  ...
  void updatePatientById_shouldReturnOk() throws Exception {
    ...
    when(patientService.updatePatientById(eq(patientId), any(PatientRequest.class)))
        .thenReturn(response);
    ...
    verify(patientService).updatePatientById(eq(patientId), any(PatientRequest.class));
  }
  ...
  void deletePatientById_shouldReturnNoContent() throws Exception {
    ...
    when(patientService.deletePatientById(patientId)).thenReturn(new PatientResponse());

    mockMvc
        .perform(delete("/api/v1/patients/{id}", patientId).with(csrf()))
