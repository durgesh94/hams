    ...
    return (root, query, criteriaBuilder) -> {
      ...
      if (filter.getDoctorId() != null) {
        predicates =
            criteriaBuilder.and(
                predicates,
                criteriaBuilder.equal(root.get("doctor").get("id"), filter.getDoctorId()));
      }
      ...
      if (filter.getPatientId() != null) {
        predicates =
            criteriaBuilder.and(
                predicates,
                criteriaBuilder.equal(root.get("patient").get("id"), filter.getPatientId()));
      }

      if (filter.getDate() != null) {
        predicates =
            criteriaBuilder.and(
                predicates, criteriaBuilder.equal(root.get("appointmentDate"), filter.getDate()));
      }

      if (filter.getStatus() != null) {
        predicates =
            criteriaBuilder.and(
                predicates, criteriaBuilder.equal(root.get("status"), filter.getStatus()));
      }
